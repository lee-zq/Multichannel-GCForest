...
class gcForest(object):
    ...
    def __init__(self, shape_1X=None, n_mgsRFtree=30, window=None, stride=1,
        ...
        :param shape_1X: int or tuple list or np.array (default=None)
            Shape of a single sample element [n_rows, n_cols, n_channels]. Required when calling mg_scanning!
        ...
        """
        setattr(self, 'shape_1X', shape_1X)  # shape=[n_rows,n_cols, n_channels]
    ...
    def fit(self, X, y):  # X shape is n*20*20*3
        """ Training the gcForest on input data X and associated target y.

        :param X: np.array
            Array containing the input samples.
            Must be of shape [n_samples, data] where data is a 3D array.
    ...
    def window_slicing_pred_prob(self, X, window, shape_1X, y=None):
        ...
        :param X: np.array
            Array containing the input samples.
            Must be of shape [n_samples, data]
    ...
    def _window_slicing_img(self, X, window, shape_1X, y=None, stride=1):
        ...
        len_iter_x = np.floor_divide((shape_1X[1] - window), stride) + 1   # 每行每列扫描个数
        ...
        inds_to_take = [ref_ind + ix + shape_1X[1] * iy
                        for ix, iy in itertools.product(iterx_array, itery_array)] # 每张图中mgs采样的m个样本的索引
        # sliced_imgs = np.take(X, inds_to_take, axis=1).reshape(-1, window**2)
        
        '''
        This part for mutil-channels MGS
        '''
        SLI = np.array([]) 
        for i in range(X.shape[3]):
            X_tmp = X[:,:,:,i].reshape(X.shape[0],-1)
            sliced_imgs = np.take(X_tmp, inds_to_take, axis=1).reshape(-1, window ** 2)
            if SLI.size == 0:
                SLI = sliced_imgs
            else:
                SLI = np.hstack((SLI,sliced_imgs))

        if y is not None:
            sliced_target = np.repeat(y, len_iter_x * len_iter_y)
        elif y is None:
            sliced_target = None
        print('sliced_imgs shape after MGS：:', SLI.shape)
        return SLI, sliced_target
